...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.post("/user", async (req, reply) => {
    const schema = z.object({
      ...
      email: z.string().email({ message: "E-mail não valido" }).min(5, { message: "o E-mail deve ter no minimo 5 caracteres" }),
    ...
    try {
      ...
      if(emailExist) {
        return reply.code(400).send({ message: "Esse E-mail já foi utilizado" });
      }
